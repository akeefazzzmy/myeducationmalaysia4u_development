...
<div class="container-fluid">
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active"><div class="sb-nav-link-icon"><i class="fa fa-eye"></i></div><a href="{{route('admin-penggunaPelajar-cariPelajarKp:show-get', $user->no_kp)}}">Papar</a></li>
        <li class="breadcrumb-item active"><div class="sb-nav-link-icon"><i class="fa fa-edit"></i></div>Kemaskini</li>
    </ol>
    <div class="container">
                        ...
                        {{-- <form action="{{route('admin-penggunaPelajar:cariPelajarKpKemaskini')}}" method="POST"> --}}
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-2">
                                        ...
                                        <select class="form-control" name="statusUser">
                                            <option value="{{$user->status_users->id}}" selected>{{$user->status_users->keterangan}}</option>
                                            <option disabled>---</option>
                                            @foreach($senaraiStatus as $key=>$status)
                                            <option value="{{$status->id}}">{{$status->keterangan}}</option>
                                            @endforeach
                                        </select>
                                        {{-- <input type="text" name="statusUser" class="form-control" value="{{$user->status_users->keterangan}}"> --}}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="row">
                                  <div class="col">
                                    <div class="form-group">
                                        <a href="{{route('admin-penggunaPelajar-cariPelajarKp:show-get', $user->no_kp)}}" class="btn btn-secondary">Kembali</a>
                                        <button type="submit" class="btn btn-success">Hantar</button>
                                        {{-- <a href="{{route('admin-penggunaPelajar-cariPelajarKp:edit', $user->no_kp)}}" class="btn btn-success">Kemaskini</a> --}}
                                    </div>
                                  </div>
                                </div>
                            </div>
